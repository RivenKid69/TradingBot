...
YAML конфигурации теперь поддерживают настройки ограничителя скорости
исходящих сигналов:
...
Эти параметры передаются в `Mediator` и затем в `SignalRateLimiter` для
контроля частоты торговых сигналов.
